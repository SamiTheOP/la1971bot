...
const token = "NzU0NDExODc4NzA4NTQzNTk5.X10W0A.WO6GLVzeKYjgqJd3mTzeTUjB034"
...
dclient.on ("message"  , async message  =>  {
   ...
   const server = message.guild;
     ...
     if (message.author.bot) return;
        ...
        message.author.send("Ight,imma send that to Blazing.My DMs r closed kiddo.If you wanna contact server/tourney support,or get tourney payment,type *!!help* inside the LA BD 1971 server.")
    ...
    if(message.content.startsWith(prefix + 'kick')){
        ...
        if(message.content.length == 0){
         ...
         if(!message.mentions.members.first()){
          ...
          await mention.send(`You have been kicked from the LA 1971 BD  server by ${message.author.username}. Reason: ${message.content} `).catch(err =>{
       ...
       if(message.content.startsWith(prefix + 'ban')){
        ...
        if(!message.mentions.members.first()){
           ...
           await mention.send(`You have been banned from the LA 1971 BD server by ${message.author.username}. Reason: ${message.content} `).catch(err => {
